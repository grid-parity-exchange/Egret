...
def declare_eq_branch_power_btheta_approx_nonlin(model, index_set, branches):
    ...
    for branch_name in con_set:
        ...
        x = branch['reactance']
        b = -1/(tau*x)
        m.eq_pf_branch[branch_name] = m.pf[branch_name] == \
...
def declare_ineq_load_shed(model, index_set):
    ...
    m.subproblem.ineq_load_shed_ub = pe.Constraint(con_set)
    m.subproblem.ineq_load_shed_lb = pe.Constraint(con_set)
    for bus_name in index_set:
        m.subproblem.ineq_load_shed_ub[bus_name] = \
            m.subproblem.load_shed[bus_name] <= m.subproblem.pl[bus_name]
        m.subproblem.ineq_load_shed_lb[bus_name] = \
            m.subproblem.pl[bus_name]*(1-m.u[bus_name]) <= m.subproblem.load_shed[bus_name]
...
def declare_ineq_gen(model, index_set, gens):
    ...
    for gen_name in index_set:
        ...
        m.subproblem.ineq_gen_ub[gen_name] = \
            m.subproblem.pg[gen_name] <= m.v[gen_name]*gen['p_max']
        m.subproblem.ineq_gen_lb[gen_name] = \
            m.v[gen_name] * gen['p_min'] <= m.subproblem.pg[gen_name] # TODO: implementation of feasible bilevel for when p_min > 0.