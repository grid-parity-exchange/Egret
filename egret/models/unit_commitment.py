...
def solve_unit_commitment(model_data,
    ...
    for g,g_dict in thermal_gens.items():
        ...
        if gfs:
            fuel_consumed = {}
        gdf = (df and (g in m.DualFuelGenerators))
        if gdf:
            aux_fuel_indicator = {}
        gafs = (fs and (g in m.AuxFuelSupplyGenerators))
        if gafs:
            aux_fuel_consumed = {}
        ...
        for dt, mt in zip(data_time_periods,m.TimePeriods):
            ...
            if g in m.DualFuelGenerators:
                commitment_cost_dict[dt] += value(m.NoLoadCost[g,mt])
            else:
                commitment_cost_dict[dt] += value(m.MinimumProductionCost[g]*m.UnitOn[g,mt]*m.TimePeriodLengthHours)
