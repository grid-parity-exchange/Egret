        ...
        if slacks is None:
            ...
            for t in t_subset:
                ...
                for bn, constr in b_.ineq_pf_branch_thermal_bounds.items():
                    slacks[bn, constr] = constr.slack()

        b_other = m.TransmissionBlock[t_o]
        PTDF_other = b_other._PTDF

        PVF[t_o], viol_lazy[t_o] = lpu.copy_active_to_next_time(m,  b_other, PTDF_other, slacks)

        logger.debug(prepend_str+"adding {0} flow constraints at time {1}".format(len(viol_lazy[t_o]),t_o))
        lpu.add_violations(viol_lazy[t_o], PVF[t_o], b_other, md, solver, ptdf_options, PTDF_other, time=t_o, prepend_str=prepend_str)
...
def _lazy_ptdf_uc_solve_loop(m, md, solver, timelimit, solver_tee=True, symbolic_solver_labels=False, iteration_limit=100000, vars_to_load=None, add_all_lazy_violations=False, warmstart_loop=False, t_subset=None, vars_to_load_t_subset=None, prepend_str=""):
    ...
    for i in range(iteration_limit):
        for t in time_periods:
            ...
            PVF[t], viol_num[t], mon_viol_num[t], viol_lazy[t] = \
        ...
        for t in time_periods:
            ...
            lpu.add_violations(viol_lazy[t], PVF[t], b, md, solver, ptdf_options, PTDF, time=t, prepend_str=prepend_str)
